...
const ToolsHub = ({ onOpenMap, onOpenFiveIdeas, onOpenHunt, onOpenCityGuides, onOpenQuickTools }) => (
    <div style={{ height: '100%', overflowY: 'auto', padding: 'var(--space-lg)', paddingTop: 'calc(var(--space-xl) + env(safe-area-inset-top, 0px))', background: 'linear-gradient(180deg, var(--color-warm-white) 0%, var(--color-cream) 100%)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-md)' }}>
            ...
            <button onClick={onOpenFiveIdeas} style={toolCardStyle}>
                <div style={{ fontSize: '34px', marginBottom: '8px' }}>✨</div>
                <div style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--color-navy)' }}>5 Ideas</div>
                <div style={{ fontSize: '0.72rem', color: 'var(--color-text-light)', marginTop: '3px' }}>What should we do?</div>
            </button>
...
const App = () => {
    ...
    const handleTabChange = (tab) => {
        if (tab !== activeTab) { setPlanView('chat'); setFeedView('feed'); setToolsView('hub') }
        setActiveTab(tab); setFabOpen(false)
    }
    ...
    const handleFingerprintSuccess = () => {
        setShowFingerprintModal(false); setIsAdmin(true)
        if (!currentUser) {
        ...
        setScreen('explorer'); setActiveTab('plan'); setPlanView('dashboard')
    }
    ...
    const handleRefineIdea = (idea) => {
        ...
        if (userId !== currentUser) { setCurrentUser(userId); try { localStorage.setItem('ep26_currentUser', userId) } catch {} }
        setActiveTab('plan'); setPlanView('chat')
    }
    ...
    const handleDeleteIdea   = (ideaId) => { if (confirm('Delete this saved idea?')) store.deleteSavedIdea(ideaId) }
    ...
    return (
        <div style={{ height: '100%' }}>
            <SheetsStatusBanner error={!bannerDismissed && store.sheetsError ? store.sheetsError : null} onDismiss={() => setBannerDismissed(true)} />
            ...
            {screen === 'familyLanding' && (
                <FamilyLandingScreen userProfiles={store.userProfiles} activities={store.activities} onSelectUser={handleSelectUser} onOpenAdmin={() => setShowFingerprintModal(true)} onBack={() => setScreen('home')} euroLedger={store.euroLedger} />
            )}
            ...
            {screen === 'explorer' && currentUser && (
                <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ flex: 1, minHeight: 0, overflow: 'hidden', position: 'relative' }}>
                        ...
                        {activeTab === 'home' && (
                            <PersonalHomeScreen userId={currentUser} user={store.userProfiles[currentUser] || {}} activities={store.activities} euroLedger={store.euroLedger} newStoryAvailable={newStoryAvailable} onOpenDailyStories={handleOpenDailyStories} onOpenJournal={() => { setFabPendingAction('journal'); setActiveTab('memories') }} onSwitchUser={handleSwitchUser} itinerary={store.itinerary} />
                        )}
                        ...
                        {activeTab === 'feed' && feedView === 'dailyStories' && (
                            <DailyStoriesScreen onBack={() => setFeedView('feed')} journalDigest={store.journalDigest} journalEntries={store.journalEntries} itinerary={store.itinerary} currentUser={currentUser} userProfiles={store.userProfiles} onSaveStory={store.saveJournalStory} canGenerate={true} />
                        )}

                        {activeTab === 'plan' && planView === 'chat' && (
                            <ChatScreen userId={currentUser} user={store.userProfiles[currentUser]} onBack={() => setActiveTab('home')} onOpenDashboard={() => setPlanView('dashboard')} onOpenMap={() => { setToolsView('map'); setActiveTab('tools') }} store={store} />
                        )}
                        {activeTab === 'plan' && planView === 'dashboard' && (
                            <DashboardScreen onBack={() => setPlanView('chat')} activities={store.activities} savedIdeas={store.savedIdeas} bookingItems={store.bookingItems} userProfiles={store.userProfiles} isAdmin={isAdmin} currentUserId={currentUser} onApprove={handleApprove} onFeedback={handleFeedback} onRefineIdea={handleRefineIdea} onSubmitIdea={handleSubmitIdea} onDeleteIdea={handleDeleteIdea} onToggleBooked={handleToggleBooked} onDeleteBooking={handleDeleteBooking} onAddBooking={handleAddBooking} euroLedger={store.euroLedger} awardEuros={store.awardEuros} processWithdrawal={store.processWithdrawal} />
                        )}
                        ...
                        {activeTab === 'tools' && toolsView === 'fiveIdeas' && (
                            <FiveIdeasScreen onBack={() => setToolsView('hub')} itinerary={store.itinerary} />
                        )}
                    ...
                    <BottomNav activeTab={activeTab} onTabChange={handleTabChange} newStoryAvailable={newStoryAvailable} />

                    {activeTab !== 'plan' && (
                        <>
                            {fabOpen && <div onClick={() => setFabOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 888 }} />}
                            {fabOpen && (
                            ...
                            <button onClick={() => setFabOpen(f => !f)} aria-label="Quick capture" style={{ position: 'fixed', bottom: 'calc(70px + env(safe-area-inset-bottom, 0px))', right: 'var(--space-lg)', width: '44px', height: '44px', background: 'var(--color-terracotta)', color: 'white', border: 'none', borderRadius: '50%', fontSize: '24px', fontWeight: 300, lineHeight: 1, cursor: 'pointer', boxShadow: '0 3px 12px rgba(200,96,58,0.45)', zIndex: 890, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                {fabOpen ? '×' : '+'}
                            </button>
                        </>
                    )}
