...
const Lightbox = ({ entry, onClose }) => (
  <div
    ...
    <img
      src={getThumbUrl(entry.photoUrl, 'w1600')}
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '4px', marginBottom: 'var(--space-sm)' }}>
      {photos.map(entry => (
        <div
          ...
          <img
            src={getThumbUrl(entry.photoUrl, 'w400')}
            alt={entry.entryText || 'Photo'}
...
const JournalCard = ({ entry, onHeartEntry, heartedIds }) => {
...
export const FamilyFeedScreen = ({ onBack, journalEntries, onHeartEntry }) => {
          ...
          sortedDates.map(dateKey => {
            ...
            return (
              <div key={dateKey} style={{ marginBottom: 'var(--space-xl)', animation: 'slideUp 0.4s ease-out' }}>
                ...
                <PhotoGrid photos={photos} onPhotoTap={setLightboxEntry} />
                {journals.map(entry => (
